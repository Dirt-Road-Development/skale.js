...
describe("Unit Testing for Proof of Work", () => {
  ...
  const setupFixture = async() => {
    const pow = new AnonymousPoW({ rpcUrl: "https://staging-v3.skalenodes.com/v1/staging-fast-active-bellatrix" });
    return { pow };
  }
  ...
  describe("Successful Calls", () => {
    ...
    test("Calypso sFUEL Facuet - Anon - Encoded", async() => {
      const { pow } = await setupFixture();
      const receipt = await pow.send({
        to: "0xa9eC34461791162Cae8c312C4237C9ddd1D64336",
        data: new utils.Interface(abi).encodeFunctionData(
          "pay",
